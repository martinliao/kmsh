...
class block_enrolverifycreator_renderer extends plugin_renderer_base {
    public function manage_page($table, $manageurl, $manageui, $filterform = NULL) {
        echo $this->header();
        if($manageui){
            echo $this->heading(get_string('confirmusers', 'block_enrolverifycreator'));
            echo get_string('confirmusers_desc', 'block_enrolverifycreator');
        }else{
            echo $this->heading(get_string('applylist', 'block_enrolverifycreator'));
            echo get_string('applylist_desc', 'block_enrolverifycreator');
        }
    ...
    public function edit_page($mform) {
        echo $this->header();
        echo $this->heading(get_string('pluginname', 'block_enrolverifycreator'));
        $mform->display();
        echo $this->footer();
    }
    ...
    public function manage_form($table, $manageurl, $manageui, $filterform) {
        ...
        echo html_writer::start_tag('form', array(
            'id' => 'enrol_verify_creator_manage_form',
            'method' => 'post',
        ...
        $PAGE->requires->js_amd_inline("
            ...
            $('#enrol_verify_creator_manage_form').find('input:checkbox').prop('checked', true);
                        e.preventDefault();
            });
            
            $('#checknone').click(function(e) {
            $('#enrol_verify_creator_manage_form').find('input:checkbox').prop('checked', false);
                        e.preventDefault();
            });
        });");
        ...
        if($manageui){
            echo html_writer::empty_tag('input', array(
                'type' => 'submit',
                'name' => 'confirm',
                'value' => get_string('btnagree', 'block_enrolverifycreator')));
            echo html_writer::empty_tag('input', array(
                'type' => 'submit',
                'name' => 'reject',
                'value' => get_string('btnreject', 'block_enrolverifycreator')));
        }else{
            echo html_writer::empty_tag('input', array(
                'type' => 'submit',
                'name' => 'cancel',
                'value' => get_string('btncancel', 'block_enrolverifycreator')));
        }
    ...
    public function manage_table($table, $manageui) {
        if($manageui){
            ...
            $headers = array(
                ...
                get_string('applydate', 'block_enrolverifycreator'),
                get_string('verifyuser', 'block_enrolverifycreator'));
        }else{
            $columns = array(
                'checkboxcolumn',
                'course',
...
//apply history
    public function apply_history_page($table, $manageurl, $filterform) {
        echo $this->header();
        echo $this->heading(get_string('applyhistory', 'block_enrolverifycreator'));
        echo '<div id="tablecontainer">';
    ...
    public function apply_history_table($table) {
        ...
        $headers = array(
            ...
            get_string('applydate', 'block_enrolverifycreator'),
            get_string('verifyuser', 'block_enrolverifycreator'),
            get_string('verifydate', 'block_enrolverifycreator'),
            get_string('status'),
            get_string('reason', 'block_enrolverifycreator'),
            );
        $table->define_columns($columns);
        $table->define_headers($headers);
        $table->sortable(true, 'applydate', SORT_DESC);           
...
//verify history
    public function verify_history_page($table, $manageurl, $filterform) {
        echo $this->header();
        echo $this->heading(get_string('verifyhistory', 'block_enrolverifycreator'));
        echo '<div id="tablecontainer">';
...
class block_enrolverifycreator_reject_request_form extends moodleform {
    function definition() {
        $mform =& $this->_form;
        ...
        $mform->addElement('textarea', 'reason', get_string('reason', 'block_enrolverifycreator'), array('rows'=>'1', 'cols'=>'50'));
