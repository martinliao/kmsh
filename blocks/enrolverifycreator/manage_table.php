...
class blocks_enrolverifycreator_manage_table extends table_sql {
    public function __construct($verifyuser = null, $filters = array()) {
        parent::__construct('block_enrolverifycreator_manage_table');
        ...
        if(is_siteadmin($USER)){
            ...
            $sqlwhere = 'n.status = 0 AND n.enrolid = e.id AND (n.verifyuser like :censor1 OR n.verifyuser like :censor2 OR verifyuser = :verifyuser)';
            $sqlparams = array('censor1'=>$like1, 'censor2'=>$like2, 'verifyuser'=>$USER->id);
        }
        
        if(!empty($filters['user'])){
            $sqlwhere .= ' AND u.lastname like "%'.$filters['user'].'%"';
        }
    ...
    public function col_applydate($row) {
        return block_enrolverifycreator_get_date_format($row->applydate);
    }
}

class blocks_enrolverifycreator_apply_table extends table_sql {
    public function __construct($verifyuser = null) {
        parent::__construct('block_enrolverifycreator_apply_table');
...
class blocks_enrolverifycreator_applyhistory_table extends table_sql {
    public function __construct($userid = null, $filters = array()) {
        parent::__construct('block_enrolverifycreator_applyhistory_table');
    ...
    public function col_status($row) {
        return block_enrolverifycreator_get_verify_status($row->status);
    }
    public function col_startdate($row) {
        return date("Y-m-d", $row->startdate);
    }
    ...
    public function col_verifydate($row) {
        return block_enrolverifycreator_get_date_format($row->verifydate);
    }
}

class blocks_enrolverifycreator_verifyhistory_table extends table_sql {
    public function __construct($verifyuser = null, $filters = array()) {
        parent::__construct('block_enrolverifycreator_verifyhistory_table');
