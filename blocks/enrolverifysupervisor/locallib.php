 ...
 */
 ...
 function block_enrolverifysupervisor_get_history_count($userid){
     global $DB;
     $SQL = "SELECT count(n.id)
            FROM {enrol_supervisor} n
 ...
 function block_enrolverifysupervisor_get_history_list($userid, $page, $perpage){
     global $DB;
     $before = $page*$perpage;
  ...
  function block_enrolverifysupervisor_get_mamage_history_count($userid){
     global $DB;
     $SQL = "SELECT count(n.id)
            FROM {enrol_supervisor} n
            LEFT JOIN {user} u ON u.id = n.userid
 ...
 function block_enrolverifysupervisor_get_mamage_history_list($userid, $page, $perpage){
     global $DB;
     $before = $page*$perpage;
...
function block_enrolverifysupervisor_get_verify_status($verify_status){
    $status = "";
    switch($verify_status){
        case 1:
            $status = get_string('agree', 'block_enrolverifysupervisor');
            break;
        case 2:
            $status = get_string('reject', 'block_enrolverifysupervisor');
            break;
        case 3:
            $status = get_string('cancel', 'block_enrolverifysupervisor');
            break;
        default:
            break;
    }
...
function block_enrolverifysupervisor_get_date_format($date){
