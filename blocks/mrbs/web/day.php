...
if ($pview != 1) {
    ...
    echo "<b>■".get_string('areas', 'block_mrbs')."</b><br>";
    ...
    if ($area_list_format == "select") {
        ...
        foreach ($areas as $dbarea) {
            ...
            if ($dbarea->id == $area) {
                echo "<font color=\"blue\">".s($dbarea->area_name)."</font></a><br>\n";
            } else {
    ...
    echo "<td width=\"20%\"><h3>".get_string('findroom', 'block_mrbs')."</h3>
        <form action='$gotoroom' method='get'>
            <input type='text' size='10' name='room' value='$gotoval'>
...
if (!empty($area)) {
    $sql = "SELECT e.id AS eid, r.id AS rid, e.start_time, e.end_time, e.name, e.phone1, e.type,
            e.description
    ...
    foreach ($entries as $entry) {
        ...
        if ($entry->start_time < $am7) {
            ...
            if(!empty($entry->phone1)){
                $today[$entry->rid][date($format, $am7)]["data"] .= "(".get_string('phone').":" . $entry->phone1 . ")";
            }
            $today[$entry->rid][date($format, $am7)]["long_descr"] .= $entry->description;
        } else {
            $today[$entry->rid][date($format, $start_t)]["data"] .= $entry->name;
            if(!empty($entry->phone1)){
                $today[$entry->rid][date($format, $start_t)]["data"] .= "(".get_string('phone').":" . $entry->phone1 . ")";
            }

            //$today[$entry->rid][date($format, $start_t)]["data"] .= ' - '+ 'Jack' +'';
    ...
    if (empty($rooms)) {
        ...
        echo "<tr><th align=\"center\" width=\"1%\">".($enable_periods ? get_string('period', 'block_mrbs') : get_string('time'))."</th>";
        ...
        foreach ($rooms as $room) {
            echo "<th align=\"center\" style=\"font-size: 12pt;\" width=\"$room_column_width%\">
                ...
                if (!isset($id)) {
                    ...
                    if ($pview != 1) {
                        if (!$room->allowedtobook) {
                            ...
                            echo '<img src="'.$OUTPUT->image_url('new', 'block_mrbs').'" width="16" height="16" border="0"></a>';
                            echo "</center>";
                            if ($javascript_cursor) {
                                echo "<SCRIPT language=\"JavaScript\">\n<!--\n";
                                echo "EndActiveCell();\n";
