...
/// Print the header
$strtitle = get_string('manage', 'block_mrbs');
$PAGE->set_context($context);
$PAGE->navbar->add($strtitle);
$PAGE->set_pagelayout('incourse');
$PAGE->set_title($strtitle);
$PAGE->set_heading(format_string($strtitle));
...
if ($room > 0) {
    ...
    if ($change_room && $valid_email && $valid_email2) {
        ...
        //$area = $DB->get_field('block_mrbs_room', 'area_id', array('id' => $room));
        //$returnurl = new moodle_url('/blocks/mrbs/web/admin.php', array('area'=>$area));
        //redirect($returnurl);
    }
    ...
    echo '<TR><TD>'.get_string('description').'</TD><TD><textarea name="description" cols="25" rows="3">'.($dbroom->description).'</textarea></TD></TR>';
