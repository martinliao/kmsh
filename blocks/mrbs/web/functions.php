...
function print_header_mrbs($day = null, $month = null, $year = null, $area = null, $userview = false) //if values are not passed assume NULL
{
    ...
    if ($pview != 1) {
        ...
        $titlestr = get_string('pluginname', 'block_mrbs');
...
HTML2END;
        ...
        if (!$userview) {
            ...
            */
            ...
            echo '<INPUT TYPE=TEXT   NAME="search_str" VALUE="'.$search_str.'" SIZE=10>
...
function period_date_string($t, $mod_time = 0) {
    ...
    return array($p_num, userdate($t, "%Y/%m/%d ").$periods[$p_num]);
}
...
function time_date_string($t) {
    ...
    if ($twentyfourhour_format) {
        return userdate($t, "%Y/%m/%d %H:%M:%S");
    } else {
        return userdate($t, "%Y/%m/%d %I:%M:%S%p");
    }
}
