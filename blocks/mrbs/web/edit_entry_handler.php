    ...
    foreach ($rooms as $room_id) {
        if ($edit_type == "series") {
            ...
            $rep_details = mrbsCreateRepeatingEntrys($starttime, $endtime, $rep_type, $rep_enddate, $rep_opt,
                                                     $room_id, $create_by, $name, $phone1, $type, $description,
            ...
            $new_id = mrbsCreateSingleEntry($starttime, $endtime, $entry_type, $repeat_id, $room_id,
                                            $create_by, $name, $phone1, $type, $description, $id, $roomchange);
...
if (strlen($err)) {
    ...
    echo "<H1><B><font color = 'red'>".get_string('sched_conflict', 'block_mrbs')."</font></B></H1>";
    if (!isset($hide_title)) {
