...
function minicals($year, $month, $day, $area, $room, $dmy, $usertt = false) {
    ...
    class MRBSCalendar {
        ...
        function getFirstDays() {
            ...
            // use calendar/lib function
            $calendartype = \core_calendar\type_factory::get_calendar_instance();
            $daynames = $calendartype->get_weekdays();
            $s = "";
            foreach($daynames as $pos => $dn){
                $s .= '<th align=center valign=top class=\"calendarHeader\"><abbr title="'. $daynames[$pos]['fullname'] .'">'.
    ...
    echo "<td>";
    $cal = new MRBSCalendar(date("d", $lastmonth), date("m", $lastmonth), date("Y", $lastmonth), 0, $area, $room, $dmy, $usertt);
    echo $cal->getHTML();
    echo "</td>";

    echo "<td>";
    $cal = new MRBSCalendar(date("d", $thismonth), date("m", $thismonth), date("Y", $thismonth), 1, $area, $room, $dmy, $usertt);
    echo $cal->getHTML();
    echo "</td>";

    echo "<td>";
    $cal = new MRBSCalendar(date("d", $nextmonth), date("m", $nextmonth), date("Y", $nextmonth), 0, $area, $room, $dmy, $usertt);
    echo $cal->getHTML();
    echo "</td>";
}
