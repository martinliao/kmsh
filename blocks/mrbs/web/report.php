...
$downloadform = optional_param('downloadform', false, PARAM_TEXT);
...
$sumby = optional_param('sumby', 'c', PARAM_ALPHA);
...
function date_time_string($t) {
    ...
    return userdate($t, "%Y/%m/%d ".$timeformat);
}
...
function describe_span($starts, $ends) {
    $start_date = userdate($starts, '%Y/%m/%d');
...
function reporton(&$item, &$last_area_room, &$last_date, $sortby, $display) {
    ...
    $date = userdate($item->start_time, "%Y/%m/%d");
...
function cell($count, $hours) {
    ...
    echo "<td class=\"BR\" align=right>($count) "
        .sprintf("%.2f", $hours)."</td>\n";
    */
    echo "<td class=\"BR\" align=right>($count) ". $hours ."</td>\n";
}
...
function do_summary(&$count, &$hours, &$room_hash, &$name_hash) {
    ...
    foreach($room_hash as $key => $name) {
        $rooms[] = $key;
    }
    ksort($rooms);
    reset($name_hash);
    //while (list($name_key) = each($name_hash)) {
    foreach($name_hash as $key => $name) {
        $names[] = $key;
    }
    ksort($names);
    ...
    echo "<td class=\"BL\" align=left><br><b>".get_string('total')."</b></td></tr>\n";
    $grand_count_total = 0;
    $grand_hours_total = 0;
    ...
    for ($r = 0; $r < $n_names; $r++) {
        ...
        $row_hours_total = 0.0;
        $name = $names[$r];
        echo "<tr><td class=\"BL\" align=left><b>$name</b></td>\n";
        for ($c = 0; $c < $n_rooms; $c++) {
            $room = $rooms[$c];
    ...
    echo "<tr><td class=\"BL\" align=left><b>".get_string('total')."</b></td>\n";
    for ($c = 0; $c < $n_rooms; $c++) {
...
if (!$downloadform) {
    ...
    require_capability('block/mrbs:administermrbs', $context);

    // print the page header
    print_header_mrbs($day, $month, $year, $area);
}

if ($submitform OR $downloadform) {
...
if($summarize == 2){
?>
                <td colspan="2" align="center"><input name="downloadform" type="submit"
                                                      value="<?php echo get_string('downloadquery', 'block_mrbs') ?>">
<?php
}
?>                
                </td>
...
if ($downloadform) {
    ...
    if ($nmatch == 0) {
        ...
        $filename = get_string('filename', 'block_mrbs').'_'.userdate(time(),'%Y%m%d',99,false);
        ...
        foreach ($rep as $item) {
            if ($summarize == 2) {
                ...
                $duration = describe_period_span($item->start_time, $item->end_time);

                /* show booking lists */
                if ($sumby == "d") {
                    $namekey = s($item->description);
                } else {
                    $namekey = s($item->create_by);
                }
                $roomkey = s($item->area_name)."<br>".s($item->room_name);

                $detail_user[$roomkey][$namekey][] = $duration;
                $detail_desc[$roomkey][$namekey][] = $duration;
            }
        }
        ...
        if ($summarize == 2) {
            ...
            $headdata = [''];
            for ($c = 0; $c < $n_rooms; $c++) {
                $headdata[] = strip_tags($rooms[$c]);
                $col_count_total[$c] = 0;
                $col_hours_total[$c] = 0.0;
            }
            $headdata[] = get_string('total');
            fputcsv($fp, $headdata);
            ...
            for ($r = 0; $r < $n_names; $r++) {
                ...
                for ($c = 0; $c < $n_rooms; $c++) {
                    ...
                    if (isset($count[$room][$name])) {
                        ...
                        $descript = "";
                        if($sumby == 'd'){
                            $descript = "\r\n".implode("\r\n", $detail_desc[$room][$name]);
                        }else if($sumby == 'c'){
                            $descript = "\r\n".implode("\r\n", $detail_user[$room][$name]);
                        }
                        $linedata[] = '('.$count_val.')' . $hours_val . $descript;
                ...
                $linedata[] = '('.$row_count_total.')' . $row_hours_total;
                fputcsv($fp, $linedata);
            ...
            $totaldata = [get_string('total')];
            for ($c = 0; $c < $n_rooms; $c++) {
                $totaldata[] = '('.$col_count_total[$c].')' . $col_hours_total[$c];
            }
            $totaldata[] = '('.$grand_count_total.')' . $grand_hours_total;
            fputcsv($fp, $totaldata);
        }
        fclose($fp);
        exit();
    }
