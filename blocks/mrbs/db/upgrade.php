...
function xmldb_block_mrbs_upgrade($oldversion=0) {
    ...
    if ($oldversion < 2016101701) {
        // Define field phone1 to be added to mrbs_entry
        $table = new xmldb_table('block_mrbs_entry');
        $field = new xmldb_field('phone1', XMLDB_TYPE_CHAR, '80', XMLDB_UNSIGNED, null, null, null, 'name');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        upgrade_block_savepoint(true, 2016101701, 'mrbs');
    }

    if ($oldversion < 2016101709) {
        $table = new xmldb_table('block_mrbs_room');
        ...
        upgrade_block_savepoint(true, 2016101709, 'mrbs');
    }
