 ...
 */
namespace block_smartmenu\local\service;
...
class content_item_service {
     ...
     */
    public function get_content_items_for_user_in_course(\stdClass $user, \stdClass $course, array $allowtypes = [], array $linkparams = []): array {
        global $PAGE;
        ...
        foreach ($allcontentitems as $contentitem) {
            ...
            if (in_array($contentitem->get_name(), $allowtypes)) {
                $allowitems[] = $contentitem;
        ...
        $availablecontentitems = array_filter($allowitems, function($contentitem) use ($course, $user, $parents) {
