...
/**
 * This file keeps track of upgrades to the creator enrolment plugin
 *
...
function xmldb_enrol_creator_upgrade($oldversion) {
    global $CFG, $DB;
    ...
    if ($oldversion < 2016052301) {
        ...
        if (!empty($managerroles)) {
            // Remove wrong CAP_PROHIBIT from creator:holdkey.
            foreach ($managerroles as $role) {
                $DB->execute("DELETE
                                FROM {role_capabilities}
                               WHERE roleid = ? AND capability = ? AND permission = ?",
                        array($role->id, 'enrol/creator:holdkey', CAP_PROHIBIT));
            }
        }
        upgrade_plugin_savepoint(true, 2016052301, 'enrol', 'creator');
    }
    ...
    if ($oldversion < 2020100601) {
        ...
        upgrade_plugin_savepoint(true, 2020100601, 'enrol', 'creator');
    }
