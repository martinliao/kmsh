...
class enrol_creator_external extends external_api {
    ...
    public static function get_instance_info_parameters() {
        return new external_function_parameters(
                array('instanceid' => new external_value(PARAM_INT, 'instance id of creator enrolment plugin.'))
            );
    }

    /**
     * Return creator-enrolment instance information.
     *
     * @param int $instanceid instance id of creator enrolment plugin.
    ...
    public static function get_instance_info($instanceid) {
        ...
        // Retrieve creator enrolment plugin.
        $enrolplugin = enrol_get_plugin('creator');
    ...
    public static function enrol_user_parameters() {
        return new external_function_parameters(
            array(
                ...
                'instanceid' => new external_value(PARAM_INT, 'Instance id of creator enrolment plugin.', VALUE_DEFAULT, 0)
            )
        );
    }
    ...
    public static function enrol_user($courseid, $password = '', $instanceid = 0) {
        ...
        // Retrieve the creator enrolment plugin.
        $enrol = enrol_get_plugin('creator');
        if (empty($enrol)) {
            throw new moodle_exception('canntenrol', 'enrol_creator');
        }

        // We can expect multiple creator-enrolment instances.
        $instances = array();
        $enrolinstances = enrol_get_instances($course->id, true);
        foreach ($enrolinstances as $courseenrolinstance) {
            if ($courseenrolinstance->enrol == "creator") {
                // Instance specified.
            ...
            if ($enrolstatus === true) {
                if ($instance->password and $params['password'] !== $instance->password) {
                    ...
                    if ($instance->customint1) {
                        ...
                        if (!enrol_creator_check_group_enrolment_key($course->id, $params['password'])) {
                            $warnings[] = array(
                                'item' => 'instance',
                                'itemid' => $instance->id,
                                'warningcode' => '2',
                                'message' => get_string('passwordinvalid', 'enrol_creator')
                            );
                            continue;
                        }
                    } else {
                        if ($enrol->get_config('showhint')) {
                            ...
                            $warnings[] = array(
                                ...
                                'itemid' => $instance->id,
                                'warningcode' => '3',
                                'message' => s(get_string('passwordinvalidhint', 'enrol_creator', $hint)) // message is PARAM_TEXT.
                            );
                            continue;
                        } else {
                            $warnings[] = array(
                ...
                $enrol->enrol_creator($instance, (object) $data);
                $enrolled = true;
                break;
            } else {
                $warnings[] = array(
