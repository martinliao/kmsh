 ...
 * @since  Moodle 3.0
 */
function enrol_creator_check_group_enrolment_key($courseid, $enrolpassword) {
    global $DB;
...
class enrol_creator_enrol_form extends moodleform {
    protected $instance;
    protected $toomany = false;

    /**
     * Overriding this function to get unique form id for multiple creator enrolments.
     *
    ...
    public function definition() {
        ...
        //$this->_form->addElement('static', 'info', '', get_string('enrolme', 'enrol_creator', $heading));
        
        //$this->add_action_buttons(false, get_string('enrolme2', 'enrol_creator', $heading));
        if(isset($instance->customint5_3) && !empty($instance->customint5_3)){
            ...
            $mform->addElement('static', 'enrolmentduration', '', get_string('realstartdatetext', 'enrol_creator', $realdate));
        }
        
        if ($instance->password) {
            // Change the id of creator enrolment key input as there can be multiple self enrolment methods.
            $mform->addElement('password', 'enrolpassword', get_string('password', 'enrol_creator'),
                    array('id' => 'enrolpassword_'.$instance->id));
            $context = context_course::instance($this->instance->courseid);
            $keyholders = get_users_by_capability($context, 'enrol/creator:holdkey', user_picture::fields('u'));
            $keyholdercount = 0;
            foreach ($keyholders as $keyholder) {
                $keyholdercount++;
                if ($keyholdercount === 1) {
                    $mform->addElement('static', 'keyholder', '', get_string('keyholder', 'enrol_creator'));
                }
            ...
            //$mform->addElement('static', 'nokey', '', get_string('nopassword', 'enrol_creator'));
        }
        ...
        $this->add_action_buttons(false, get_string('enrolme', 'enrol_creator'));
        ...
        if ($instance->password) {
            if ($data['enrolpassword'] !== $instance->password) {
                if ($instance->customint1) {
                    ...
                    if (!enrol_creator_check_group_enrolment_key($instance->courseid, $data['enrolpassword'])) {
                        // We can not hint because there are probably multiple passwords.
                        $errors['enrolpassword'] = get_string('passwordinvalid', 'enrol_creator');
                    }
                    ...
                    if ($plugin->get_config('showhint')) {
                        ...
                        $errors['enrolpassword'] = get_string('passwordinvalidhint', 'enrol_creator', $hint);
                    } else {
                        $errors['enrolpassword'] = get_string('passwordinvalid', 'enrol_creator');
                    }
                }
