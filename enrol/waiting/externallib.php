...
/**
 * Waiting enrol plugin external functions
 *
...
class enrol_waiting_external extends external_api {
    ...
    public static function get_instance_info_parameters() {
        return new external_function_parameters(
                array('instanceid' => new external_value(PARAM_INT, 'instance id of waiting enrolment plugin.'))
            );
    }

    /**
     * Return waiting-enrolment instance information.
     *
     * @param int $instanceid instance id of waiting enrolment plugin.
    ...
    public static function get_instance_info($instanceid) {
        ...
        // Retrieve waiting enrolment plugin.
        $enrolplugin = enrol_get_plugin('waiting');
    ...
    public static function enrol_user_parameters() {
        return new external_function_parameters(
            array(
                ...
                'instanceid' => new external_value(PARAM_INT, 'Instance id of waiting enrolment plugin.', VALUE_DEFAULT, 0)
            )
        );
    }
    ...
    public static function enrol_user($courseid, $password = '', $instanceid = 0) {
        ...
        // Retrieve the waiting enrolment plugin.
        $enrol = enrol_get_plugin('waiting');
        if (empty($enrol)) {
            throw new moodle_exception('canntenrol', 'enrol_waiting');
        }

        // We can expect multiple waiting-enrolment instances.
        $instances = array();
        $enrolinstances = enrol_get_instances($course->id, true);
        foreach ($enrolinstances as $courseenrolinstance) {
            if ($courseenrolinstance->enrol == "waiting") {
                // Instance specified.
        ...
        foreach ($instances as $instance) {
            $enrolstatus = $enrol->can_waiting_enrol($instance);
            if ($enrolstatus === true) {
                if ($instance->password and $params['password'] !== $instance->password) {
                    ...
                    if ($instance->customint1) {
                        ...
                        if (!enrol_waiting_check_group_enrolment_key($course->id, $params['password'])) {
                            $warnings[] = array(
                                'item' => 'instance',
                                'itemid' => $instance->id,
                                'warningcode' => '2',
                                'message' => get_string('passwordinvalid', 'enrol_waiting')
                            );
                            continue;
                        }
                    } else {
                        if ($enrol->get_config('showhint')) {
                            ...
                            $warnings[] = array(
                                ...
                                'itemid' => $instance->id,
                                'warningcode' => '3',
                                'message' => s(get_string('passwordinvalidhint', 'enrol_waiting', $hint)) // message is PARAM_TEXT.
                            );
                            continue;
                        } else {
                            $warnings[] = array(
                ...
                $enrol->enrol_waiting($instance, (object) $data);
                $enrolled = true;
                break;
            } else {
                $warnings[] = array(
