...
require_once($CFG->dirroot . '/enrol/waiting/externallib.php');

class enrol_waiting_external_testcase extends externallib_advanced_testcase {
    ...
    public function test_get_instance_info() {
        ...
        // Check if waiting enrolment plugin is enabled.
        $waitingplugin = enrol_get_plugin('waiting');
        $this->assertNotEmpty($waitingplugin);
        ...
        $instanceid1 = $waitingplugin->add_instance($course, array('status' => ENROL_INSTANCE_ENABLED,
                                                                'name' => 'Test instance 1',
                                                                'customint6' => 1,
                                                                'roleid' => $studentrole->id));
        $instanceid2 = $waitingplugin->add_instance($course, array('status' => ENROL_INSTANCE_DISABLED,
                                                                'customint6' => 1,
                                                                'name' => 'Test instance 2',
                                                                'roleid' => $studentrole->id));

        $instanceid3 = $waitingplugin->add_instance($course, array('status' => ENROL_INSTANCE_ENABLED,
                                                                'roleid' => $studentrole->id,
                                                                'customint6' => 1,
        ...
        $this->setAdminUser();
        $instanceinfo1 = enrol_waiting_external::get_instance_info($instanceid1);
        $instanceinfo1 = external_api::clean_returnvalue(enrol_waiting_external::get_instance_info_returns(), $instanceinfo1);

        $this->assertEquals($instanceid1, $instanceinfo1['id']);
        $this->assertEquals($course->id, $instanceinfo1['courseid']);
        $this->assertEquals('waiting', $instanceinfo1['type']);
        ...
        $instanceinfo2 = enrol_waiting_external::get_instance_info($instanceid2);
        $instanceinfo2 = external_api::clean_returnvalue(enrol_waiting_external::get_instance_info_returns(), $instanceinfo2);
        $this->assertEquals($instanceid2, $instanceinfo2['id']);
        $this->assertEquals($course->id, $instanceinfo2['courseid']);
        $this->assertEquals('waiting', $instanceinfo2['type']);
        $this->assertEquals('Test instance 2', $instanceinfo2['name']);
        $this->assertEquals(get_string('canntenrol', 'enrol_waiting'), $instanceinfo2['status']);
        $this->assertFalse(isset($instanceinfo2['enrolpassword']));

        $instanceinfo3 = enrol_waiting_external::get_instance_info($instanceid3);
        $instanceinfo3 = external_api::clean_returnvalue(enrol_waiting_external::get_instance_info_returns(), $instanceinfo3);
        $this->assertEquals($instanceid3, $instanceinfo3['id']);
        $this->assertEquals($course->id, $instanceinfo3['courseid']);
        $this->assertEquals('waiting', $instanceinfo3['type']);
        $this->assertEquals('Test instance 3', $instanceinfo3['name']);
        $this->assertTrue($instanceinfo3['status']);
        $this->assertEquals(get_string('password', 'enrol_waiting'), $instanceinfo3['enrolpassword']);

        // Try to retrieve information using a normal user for a hidden course.
        $user = self::getDataGenerator()->create_user();
        $this->setUser($user);
        try {
            enrol_waiting_external::get_instance_info($instanceid3);
    ...
    public function test_enrol_user() {
        ...
        $waitingplugin = enrol_get_plugin('waiting');
        $studentrole = $DB->get_record('role', array('shortname' => 'student'));
        $instance1id = $waitingplugin->add_instance($course1, array('status' => ENROL_INSTANCE_ENABLED,
                                                                'name' => 'Test instance 1',
                                                                'customint6' => 1,
                                                                'roleid' => $studentrole->id));
        $instance2id = $waitingplugin->add_instance($course2, array('status' => ENROL_INSTANCE_DISABLED,
                                                                'customint6' => 1,
                                                                'name' => 'Test instance 2',
                                                                'roleid' => $studentrole->id));
        ...
        // Self enrol me.
        $result = enrol_waiting_external::enrol_user($course1->id);
        $result = external_api::clean_returnvalue(enrol_waiting_external::enrol_user_returns(), $result);
        ...
        try {
            enrol_waiting_external::enrol_user($course2->id);
        } catch (moodle_exception $e) {
        ...
        $result = enrol_waiting_external::enrol_user($course2->id);
        $result = external_api::clean_returnvalue(enrol_waiting_external::enrol_user_returns(), $result);
        ...
        $result = enrol_waiting_external::enrol_user($course2->id, 'invalidkey');
        $result = external_api::clean_returnvalue(enrol_waiting_external::enrol_user_returns(), $result);
        self::assertFalse($result['status']);
        self::assertCount(1, $result['warnings']);
        self::assertEquals('4', $result['warnings'][0]['warningcode']);
        ...
        $waitingplugin->set_config('showhint', true);
        $result = enrol_waiting_external::enrol_user($course2->id, 'invalidkey');
        $result = external_api::clean_returnvalue(enrol_waiting_external::enrol_user_returns(), $result);
        self::assertFalse($result['status']);
        self::assertCount(1, $result['warnings']);
        ...
        $result = enrol_waiting_external::enrol_user($course2->id, 'abcdef');
        $result = external_api::clean_returnvalue(enrol_waiting_external::enrol_user_returns(), $result);
        ...
        // Now, everything ok.
        $result = enrol_waiting_external::enrol_user($course2->id, 'zyx');
        $result = external_api::clean_returnvalue(enrol_waiting_external::enrol_user_returns(), $result);
        ...
        $instance3id = $waitingplugin->add_instance($course2, array('status' => ENROL_INSTANCE_ENABLED,
                                                                'customint6' => 1,
                                                                'name' => 'Test instance 2',
                                                                'roleid' => $studentrole->id));
        ...
        self::setUser($user4);
        $result = enrol_waiting_external::enrol_user($course2->id, 'abcdef', $instance3id);
        $result = external_api::clean_returnvalue(enrol_waiting_external::enrol_user_returns(), $result);
        self::assertTrue($result['status']);
        self::assertTrue(is_enrolled($context2, $user3));
