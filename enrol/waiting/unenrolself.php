...
/**
 * Waiting enrolment plugin - support for user self unenrolment.
 *
...
$PAGE->set_url('/enrol/waiting/unenrolself.php', array('enrolid'=>$instance->id));
...
if($plugin->allow_unenrol($instance)){
    ...
    $message = get_string('unenrolselfconfirm', 'enrol_waiting', format_string($course->fullname));
    echo $OUTPUT->confirm($message, $yesurl, $nourl);
}else{
    echo $OUTPUT->box(html_writer::tag('h3',  get_string('notification_unenroldateexpired', 'enrol_waiting', date('Y/m/d H:i', $instance->unenrolenddate)), array('style' => 'text-align:center')), 'generalbox');
