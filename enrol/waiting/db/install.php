...
function xmldb_enrol_waiting_install() {
    global $CFG, $DB;
    $dbman = $DB->get_manager();

    //transfer enrol_kmsh to enrol_waiting
    $DB->execute("UPDATE {enrol} SET enrol = 'waiting' WHERE enrol = 'kmsh'");

    set_config('requirepassword', 0, 'enrol_waiting');
    set_config('usepasswordpolicy', 0, 'enrol_waiting');
    set_config('showhint', 0, 'enrol_waiting');
    set_config('groupkey', 0, 'enrol_waiting');
