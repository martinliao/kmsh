...
/**
 * This file keeps track of upgrades to the waiting enrolment plugin
 *
...
function xmldb_enrol_waiting_upgrade($oldversion) {
    global $CFG, $DB;
    ...
    if ($oldversion < 2016052301) {
        ...
        if (!empty($managerroles)) {
            // Remove wrong CAP_PROHIBIT from waiting:holdkey.
            foreach ($managerroles as $role) {
                $DB->execute("DELETE
                                FROM {role_capabilities}
                               WHERE roleid = ? AND capability = ? AND permission = ?",
                        array($role->id, 'enrol/waiting:holdkey', CAP_PROHIBIT));
            }
        }
        upgrade_plugin_savepoint(true, 2016052301, 'enrol', 'waiting');
    }
    ...
    if ($oldversion < 2020100601) {
        ...
        upgrade_plugin_savepoint(true, 2020100601, 'enrol', 'waiting');
    }
