...
class enrol_supervisor_external extends external_api {
    ...
    public static function get_instance_info_parameters() {
        return new external_function_parameters(
                array('instanceid' => new external_value(PARAM_INT, 'instance id of supervisor  enrolment plugin.'))
            );
    }

    /**
     * Return supervisor-enrolment instance information.
     *
     * @param int $instanceid instance id of supervisor  enrolment plugin.
    ...
    public static function get_instance_info($instanceid) {
        ...
        // Retrieve supervisor  enrolment plugin.
    ...
    public static function enrol_user_parameters() {
        return new external_function_parameters(
            array(
                ...
                'instanceid' => new external_value(PARAM_INT, 'Instance id of supervisor  enrolment plugin.', VALUE_DEFAULT, 0)
            )
        );
    }
    ...
    public static function enrol_user($courseid, $password = '', $instanceid = 0) {
        ...
        // Retrieve the supervisor  enrolment plugin.
        $enrol = enrol_get_plugin('supervisor');
        if (empty($enrol)) {
            throw new moodle_exception('canntenrol', 'enrol_supervisor');
        }

        // We can expect multiple supervisor-enrolment instances.
        $instances = array();
        $enrolinstances = enrol_get_instances($course->id, true);
        foreach ($enrolinstances as $courseenrolinstance) {
            if ($courseenrolinstance->enrol == 'supervisor') {
                // Instance specified.
            ...
            if ($enrolstatus === true) {
                if ($instance->password and $params['password'] !== $instance->password) {
                    ...
                    if ($instance->customint1) {
                        ...
                        if (!enrol_supervisor_check_group_enrolment_key($course->id, $params['password'])) {
                            $warnings[] = array(
                                'item' => 'instance',
                                'itemid' => $instance->id,
                                'warningcode' => '2',
                                'message' => get_string('passwordinvalid', 'enrol_supervisor')
                            );
                            continue;
                        }
                    } else {
                        if ($enrol->get_config('showhint')) {
                            ...
                            $warnings[] = array(
                                ...
                                'itemid' => $instance->id,
                                'warningcode' => '3',
                                'message' => s(get_string('passwordinvalidhint', 'enrol_supervisor', $hint)) // message is PARAM_TEXT.
                            );
                            continue;
                        } else {
                            $warnings[] = array(
                ...
                $enrol->enrol_supervisor($instance, (object) $data);
                $enrolled = true;
                break;
            } else {
                $warnings[] = array(
