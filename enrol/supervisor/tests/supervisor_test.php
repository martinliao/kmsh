...
global $CFG;
require_once($CFG->dirroot.'/enrol/supervisor/lib.php');
require_once($CFG->dirroot.'/enrol/supervisor/locallib.php');

class enrol_supervisor_testcase extends advanced_testcase {

    public function test_basics() {
        $this->assertTrue(enrol_is_enabled('supervisor'));
        $plugin = enrol_get_plugin('supervisor');
        $this->assertInstanceOf('enrol_supervisor_plugin', $plugin);
        $this->assertEquals(1, get_config('enrol_supervisor', 'defaultenrol'));
        $this->assertEquals(ENROL_EXT_REMOVED_KEEP, get_config('enrol_supervisor', 'expiredaction'));
    }

    public function test_sync_nothing() {
        global $SITE;

        $enrolplugin = enrol_get_plugin('supervisor');
    ...
    public function test_longtimnosee() {
        ...
        $this->assertEquals(3, $DB->count_records('enrol', array('enrol'=>'supervisor')));
        $instance1 = $DB->get_record('enrol', array('courseid'=>$course1->id, 'enrol'=>'supervisor'), '*', MUST_EXIST);
        $instance2 = $DB->get_record('enrol', array('courseid'=>$course2->id, 'enrol'=>'supervisor'), '*', MUST_EXIST);
        $instance3 = $DB->get_record('enrol', array('courseid'=>$course3->id, 'enrol'=>'supervisor'), '*', MUST_EXIST);
    ...
    public function test_send_expiry_notifications() {
        ...
        /** @var $enrolplugin enrol_supervisor_plugin */
        $enrolplugin = enrol_get_plugin('supervisor');
        ...
        $this->assertEquals(4, $DB->count_records('enrol', array('enrol'=>'supervisor')));
        ...
        $instance4 = $DB->get_record('enrol', array('courseid'=>$course4->id, 'enrol'=>'supervisor'), '*', MUST_EXIST);
    ...
    public function test_show_enrolme_link() {
        ...
        // Cohort member test.
        $instance5 = $DB->get_record('enrol', array('courseid'=>$course5->id, 'enrol'=>'supervisor'), '*', MUST_EXIST);
        ...
        $instance7 = $DB->get_record('enrol', array('courseid'=>$course6->id, 'enrol'=>'supervisor'), '*', MUST_EXIST);
        ...
        $instance8 = $DB->get_record('enrol', array('courseid'=>$course7->id, 'enrol'=>'supervisor'), '*', MUST_EXIST);
        ...
        $instance9 = $DB->get_record('enrol', array('courseid'=>$course8->id, 'enrol'=>'supervisor'), '*', MUST_EXIST);
        ...
        $instance10 = $DB->get_record('enrol', array('courseid'=>$course9->id, 'enrol'=>'supervisor'), '*', MUST_EXIST);
        ...
        $instance11 = $DB->get_record('enrol', array('courseid'=>$course10->id, 'enrol'=>'supervisor'), '*', MUST_EXIST);
        ...
        $instance12 = $DB->get_record('enrol', array('courseid'=>$course11->id, 'enrol'=>'supervisor'), '*', MUST_EXIST);
     ...
     */
    public function test_can_supervisor_enrol() {
        ...
        $expectederrorstring = get_string('canntenrol', 'enrol_supervisor');
    ...
    /**
     * Test enrol_supervisor_check_group_enrolment_key
     */
    public function test_enrol_supervisor_check_group_enrolment_key() {
        global $DB;
        ...
        $result = enrol_supervisor_check_group_enrolment_key($course->id, 'invalidpassword');
        $this->assertFalse($result);

        $result = enrol_supervisor_check_group_enrolment_key($course->id, 'thepassword');
        ...
        $result = enrol_supervisor_check_group_enrolment_key($othercourse->id, 'thepassword');
        $this->assertFalse($result);
    ...
    public function test_get_welcome_email_contact() {
        ...
        // Enable supervisor  enrolment plugin and set to send email from course contact.
        $enrolplugin = enrol_get_plugin('supervisor');
        $instance1 = $DB->get_record('enrol', ['courseid' => $course1->id, 'enrol' => 'supervisor'], '*', MUST_EXIST);
        $instance1->customint6 = 1;
        ...
        assign_capability('enrol/supervisor:holdkey', CAP_ALLOW, $managerrole->id, $context);
    ...
    public function test_get_user_enrolment_actions() {
        ...
        // Only enable the supervisor enrol plugin.
