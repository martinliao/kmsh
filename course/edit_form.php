...
class course_edit_form extends moodleform {
    ...
    function definition() {
        ...
        $mform->setDefault('enddate', time() + (DAYSECS * 18));
        ...
        //IP限制
        $mform->addElement('textarea', 'allowedip', get_string("allowediplist", "admin"), 'wrap="virtual" rows="4" cols="50"');
        //報名開始日期
        $mform->addElement('date_time_selector', 'enrolstartdate', get_string('enrolstartdate', 'local_mooccourse'));
        $mform->addHelpButton('enrolstartdate', 'enrolstartdate', 'local_mooccourse');
        $mform->setDefault('enrolstartdate', time());
        //報名結束日期
        $mform->addElement('date_time_selector', 'enrolenddate', get_string('enrolenddate', 'local_mooccourse'));
        $mform->addHelpButton('enrolenddate', 'enrolenddate', 'local_mooccourse');
        $mform->setDefault('enrolenddate', time());
        //承辦人 promoter
        $mform->addElement('text','promoter', get_string('course_promoter', 'local_mooccourse'),'maxlength="20"  size="10"');
        $mform->setType('promoter', PARAM_RAW);
        $mform->setDefault('promoter', $USER->firstname);
        //開課單位 dept
		require_once($CFG->dirroot . '/user/profile/lib.php');
        $userprofile = (array)profile_user_record($USER->id, false);
        $mform->addElement('text','dept', get_string('course_dept', 'local_mooccourse'),'maxlength="20"  size="10"');
        $mform->setType('dept', PARAM_RAW);
        $mform->setDefault('dept', $userprofile['DeptName']);
        //講師 lector
        $lectors = [];
        if($lector_cohort = $DB->get_record('cohort', array('idnumber'=>'lector'))){
            $lectorsql = "SELECT DISTINCT u.id, u.firstname 
                          FROM {cohort_members} cm 
                          LEFT JOIN {cohort} c ON c.id = cm.cohortid
                          LEFT JOIN {user} u ON cm.userid = u.id
                          WHERE u.deleted = 0 AND u.suspended = 0 AND c.idnumber = 'lector' ";
            $lectors = array(0=>get_string('choose')) + $DB->get_records_sql_menu($lectorsql, array());
        }
        $mform->addElement('select', 'lector', get_string('course_lector', 'local_mooccourse'), $lectors);
        ...
        if ($dbman->table_exists($table)) {
            if(!empty($course->id) && $info = $DB->get_record('clickap_course_info',array('courseid'=>$course->id))){
                $course->hours = $info->hours;
            }
        }
        
        if(array_key_exists("hourcategories", $clickap)){
            $mform->addElement('header', 'clickaphdr', get_string('clickaphdr', 'clickap_base'));            
            $mform->setExpanded('clickaphdr');

            $course->hcenable = 1;
            if(empty($course->id)) {
                $course->hcenable = 1;
            }
            else if(!isset($course->hours) || (isset($course->hours) && ($course->hours == 0))){
                $course->hcenable = 0;
            }

            $choices = array();
            $choices['0'] = get_string('disable', 'clickap_hourcategories');
            $choices['1'] = get_string('enable', 'clickap_hourcategories');
            $mform->addElement('select', 'hcenable', get_string('hcvisibility', 'clickap_hourcategories'), $choices);
            $mform->setDefault('hcenable', 1);
            ...
            if(array_key_exists("longlearn_categories", $clickap)){
                ...
                $mform->hideIf('longlearn_category', 'hcenable' , 'neq', 1);
            }

            //hour categories options
            if (empty($course->id)) {
                $thisYear = date('Y', time()) - 1911;
            }else{
                $thisYear = date('Y', $course->startdate) - 1911;
            }
            ...
            $mform->hideIf('hourcategory', 'hcenable' , 'neq', 1);

            if(array_key_exists("code", $clickap)){
                if($model = $DB->get_records_menu('clickap_code', array('status'=>'1', 'type'=>'model'), 'sortorder', 'id,name')){
                    ...
                    $mform->hideIf('model', 'hcenable' , 'neq', 1);
                }
                ...
                if($credit = $DB->get_records_menu('clickap_code', array('status'=>'1', 'type'=>'credit'), 'sortorder', 'id,name')){
                    ...
                    $mform->hideIf('credit', 'hcenable' , 'neq', 1);
                }
                ...
                if($unit = $DB->get_records_menu('clickap_code', array('status'=>'1', 'type'=>'unit'), 'sortorder', 'id,name')){
                    ...
                    $mform->hideIf('unit', 'hcenable' , 'neq', 1);
                }
                ...
                if($city = $DB->get_records_menu('clickap_code', array('status'=>'1', 'type'=>'city'), 'sortorder', 'id,name')){
                    ...
                    $mform->hideIf('city', 'hcenable' , 'neq', 1);
                }
            }
    ...
    function validation($data, $files) {
        ...
        /*click-ap course hour*/
        if(isset($data['hcenable']) && $data['hcenable'] == 1){
            if(!empty($data['hours'])){
                if($data['hours'] < 0 || $data['hours'] >= 20){
