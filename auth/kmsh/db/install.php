...
function xmldb_auth_kmsh_install() {
    global $CFG, $DB;
    ...
    if(!$kmshcohort = $DB->get_record('cohort', array('idnumber'=>'kmsh'))){
        ...
        $kmshcohort->id        = cohort_add_cohort($cohort);
    }
    if(!$kmshcohort = $DB->get_record('cohort', array('idnumber'=>'lector'))){
        $kmshcohort = new stdClass();
        $kmshcohort->contextid = context_system::instance()->id;
        $kmshcohort->name      = '講師群組';
        $kmshcohort->idnumber  = 'lector';
        $kmshcohort->id        = cohort_add_cohort($cohort);
    }
