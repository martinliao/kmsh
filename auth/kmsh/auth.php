 ...
 */
class auth_plugin_kmsh extends auth_plugin_base {
    ...
    function __construct() {
        ...
        $this->authtype = 'kmsh';
        $this->config = get_config('auth_kmsh');
    ...
    function user_login($username, $password) {
        ...
        if ($this->is_internal()) {
            ...
            if (!$rs) {
                $authdb->Close();
                debugging(get_string('auth_dbcantconnect','auth_kmsh'));
                return false;
            }
     ...
     */
    function db_profile_init() {
        if ($this->is_configured() === false) {
            throw new moodle_exception('auth_dbcantconnect', 'auth_kmsh');
        }
        ...
        $authdb->charSet = "UTF8";
        $authdb->Connect($this->config->host, $this->config->user_profile, $this->config->pass_profile, $this->config->name_profile, true);
    ...
    function db_attributes() {
        ...
        $moodleattributes['username'] = "a172a.fpw001v12.員編";
    ...
    function get_userinfo($username) {
        ...
        if ($selectfields) {
            ...
            $sql = "SELECT $select
                      FROM a172a.fpw001v12
                     WHERE a172a.fpw001v12.員編 = '".$this->ext_addslashes($extusername)."'";
    ...
    function sync_users(progress_trace $trace, $do_updates=false) {
        ...
        if (!empty($this->config->removeuser)) {
            ...
            if (!empty($removeusers)) {
                $trace->output(get_string('auth_dbuserstoremove', 'auth_kmsh', count($removeusers)));

                foreach ($removeusers as $user) {
                    if ($this->config->removeuser == AUTH_REMOVEUSER_FULLDELETE) {
                        delete_user($user);
                        $trace->output(get_string('auth_dbdeleteuser', 'auth_kmsh', array('name'=>$user->username, 'id'=>$user->id)), 1);
                    } else if ($this->config->removeuser == AUTH_REMOVEUSER_SUSPEND) {
                        $updateuser = new stdClass();
                        $updateuser->id   = $user->id;
                        $updateuser->suspended = 1;
                        user_update_user($updateuser, false);
                        $trace->output(get_string('auth_dbsuspenduser', 'auth_kmsh', array('name'=>$user->username, 'id'=>$user->id)), 1);
                    }
                }
            }
            unset($removeusers);
        ...
        if ($do_updates) {
            ...
            if (!empty($updatekeys)) {
                ...
                foreach($userlistchunks as $userlistchunk) {
                    ...
                    $params['authtype'] = $this->authtype;
                    $params['mnethostid'] = $CFG->mnet_localhost_id;
                    $sql = "SELECT u.id, u.username, u.suspended, u.department
                          FROM {user} u
                ...
                if ($update_users) {
                    ...
                    foreach ($update_users as $user) {
                        $user->old_department = $user->department;
                        if ($this->update_user_record($user->username, $updatekeys, false, (bool) $user->suspended)) {
                            $trace->output(get_string('auth_dbupdatinguser', 'auth_kmsh', array('name'=>$user->username, 'id'=>$user->id)), 1);

                            // sync cohort.
                            $this->sync_cohorts($user, $user->old_department);
                        } else {
                            $trace->output(get_string('auth_dbupdatinguser', 'auth_kmsh', array('name'=>$user->username, 'id'=>$user->id))." - ".get_string('skipped'), 1);
                        }
                    }
                    unset($update_users);
                }
        ...
        if (!empty($add_users)) {
            $trace->output(get_string('auth_dbuserstoadd','auth_kmsh',count($add_users)));
            ...
            foreach($add_users as $user) {
                ...
                if ($this->config->removeuser == AUTH_REMOVEUSER_SUSPEND) {
                    if ($olduser = $DB->get_record('user', array('username' => $username, 'deleted' => 0, 'suspended' => 1,
                        ...
                        $trace->output(get_string('auth_dbreviveduser', 'auth_kmsh', array('name' => $username,
                            'id' => $olduser->id)), 1);
                        continue;
                    }
                }
                ...
                if ($collision = $DB->get_record_select('user', "username = :username AND mnethostid = :mnethostid AND auth <> :auth", array('username'=>$user->username, 'mnethostid'=>$CFG->mnet_localhost_id, 'auth'=>$this->authtype), 'id,username,auth')) {
                    $trace->output(get_string('auth_dbinsertuserduplicate', 'auth_kmsh', array('username'=>$user->username, 'auth'=>$collision->auth)), 1);
                    continue;
                }
                try {
                    $id = user_create_user($user, false, false); // It is truly a new user.
                    $trace->output(get_string('auth_dbinsertuser', 'auth_kmsh', array('name'=>$user->username, 'id'=>$id)), 1);
                    //sync cohort
                    $user->id = $id;
                    $this->sync_cohorts($user);
                } catch (moodle_exception $e) {
                    $trace->output(get_string('auth_dbinsertusererror', 'auth_kmsh', $user->username), 1);
                    continue;
                }
    ...
    function user_exists($username) {
        ...
        if (!$rs) {
            print_error('auth_dbcantconnect','auth_kmsh');
        } else if (!$rs->EOF) {
    ...
    function get_userlist() {
        ...
        // Fetch userlist.
        $rs = $authdb->Execute("SELECT a172a.fpw001v12.員編
                                  FROM a172a.fpw001v12 ");
        if (!$rs) {
            print_error('auth_dbcantconnect','auth_kmsh');
        } else if (!$rs->EOF) {
    ...
    function user_update($olduser, $newuser) {
        ...
        if (!empty($update)) {
            ...
            if (!$authdb->Execute($sql, array($this->ext_addslashes($extusername)))) {
                print_error('auth_dbupdateerror', 'auth_kmsh');
            }
        }
        $authdb->Close();
        return true;
    ...
    */
    ...
    public function test_settings() {
        ...
        if (empty($this->config->table)) {
            echo $OUTPUT->notification(get_string('auth_dbnoexttable', 'auth_kmsh'), 'notifyproblem');
            return;
        }

        if (empty($this->config->fielduser)) {
            echo $OUTPUT->notification(get_string('auth_dbnouserfield', 'auth_kmsh'), 'notifyproblem');
            return;
        }
        ...
        if (!$adodb or !$adodb->IsConnected()) {
            ...
            echo $OUTPUT->notification(get_string('auth_dbcannotconnect', 'auth_kmsh'), 'notifyproblem');
            return;
        }
        ...
        if (!$rs) {
            echo $OUTPUT->notification(get_string('auth_dbcannotreadtable', 'auth_kmsh'), 'notifyproblem');

        } else if ($rs->EOF) {
            echo $OUTPUT->notification(get_string('auth_dbtableempty', 'auth_kmsh'), 'notifyproblem');
            $rs->close();
            ...
            echo $OUTPUT->notification(get_string('auth_dbcolumnlist', 'auth_kmsh', implode(', ', $columns)), 'notifysuccess');
            $rs->close();
        }
    ...
    function sync_cohorts($user, $old_dept = "") {
        ...
        if($user){
            if(!empty($old_dept)){
                $params = array('userid' => $user->id, 'idnumber' => $old_dept);
                $sql = "SELECT cm.* FROM {cohort_members} cm
                        LEFT JOIN {cohort} c ON cm.cohortid =  c.id
                        WHERE cm.userid = :userid AND c.idnumber = :idnumber";
            ...
            if(!empty($user->department)){
                $cohortname = $userprofile['DeptName'];
                if(!$cohort = $DB->get_record('cohort', array('idnumber'=>$user->department))){
                    $cohort = new stdClass();
            ...
            if($kmshcohort = $DB->get_record('cohort', array('idnumber'=>'kmsh'))){
                if (!$DB->record_exists('cohort_members', array('cohortid'=>$kmshcohort->id, 'userid'=>$user->id))) {    
                    cohort_add_member($kmshcohort->id, $user->id);
                }
            }
        }
    }
