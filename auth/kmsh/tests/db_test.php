...
class auth_kmsh_testcase extends advanced_testcase {
    ...
    protected function init_auth_database() {
        global $DB, $CFG;
        require_once("$CFG->dirroot/auth/kmsh/auth.php");
        ...
        set_config('extencoding', 'utf-8', 'auth_kmsh');

        set_config('host', $CFG->dbhost, 'auth_kmsh');
        set_config('user', $CFG->dbuser, 'auth_kmsh');
        set_config('pass', $CFG->dbpass, 'auth_kmsh');
        set_config('name', $CFG->dbname, 'auth_kmsh');

        if (!empty($CFG->dboptions['dbport'])) {
            set_config('host', $CFG->dbhost.':'.$CFG->dboptions['dbport'], 'auth_kmsh');
        }

        switch ($DB->get_dbfamily()) {

            case 'mysql':
                set_config('type', 'mysqli', 'auth_kmsh');
                set_config('setupsql', "SET NAMES 'UTF-8'", 'auth_kmsh');
                set_config('sybasequoting', '0', 'auth_kmsh');
                if (!empty($CFG->dboptions['dbsocket'])) {
                    $dbsocket = $CFG->dboptions['dbsocket'];
                    if ((strpos($dbsocket, '/') === false and strpos($dbsocket, '\\') === false)) {
                        $dbsocket = ini_get('mysqli.default_socket');
                    }
                    set_config('type', 'mysqli://'.rawurlencode($CFG->dbuser).':'.rawurlencode($CFG->dbpass).'@'.rawurlencode($CFG->dbhost).'/'.rawurlencode($CFG->dbname).'?socket='.rawurlencode($dbsocket), 'auth_kmsh');
                }
                break;

            case 'oracle':
                set_config('type', 'oci8po', 'auth_kmsh');
                set_config('sybasequoting', '1', 'auth_kmsh');
                break;

            case 'postgres':
                set_config('type', 'postgres7', 'auth_kmsh');
                $setupsql = "SET NAMES 'UTF-8'";
                if (!empty($CFG->dboptions['dbschema'])) {
                    $setupsql .= "; SET search_path = '".$CFG->dboptions['dbschema']."'";
                }
                set_config('setupsql', $setupsql, 'auth_kmsh');
                set_config('sybasequoting', '0', 'auth_kmsh');
                if (!empty($CFG->dboptions['dbsocket']) and ($CFG->dbhost === 'localhost' or $CFG->dbhost === '127.0.0.1')) {
                    if (strpos($CFG->dboptions['dbsocket'], '/') !== false) {
                        ...
                        set_config('host', $socket, 'auth_kmsh');
                    } else {
                        set_config('host', '', 'auth_kmsh');
                    }
                }
                break;

            case 'mssql':
                set_config('type', 'mssqlnative', 'auth_kmsh');
                set_config('sybasequoting', '1', 'auth_kmsh');
                ...
                set_config('host', $dbhost, 'auth_kmsh');
                break;
        ...
        $table = new xmldb_table('auth_kmsh_users');
        ...
        set_config('table', $CFG->prefix.'auth_kmsh_users', 'auth_kmsh');
        set_config('fielduser', 'name', 'auth_kmsh');
        set_config('fieldpass', 'pass', 'auth_kmsh');
        set_config('field_map_lastname', 'lastname', 'auth_kmsh');
        set_config('field_updatelocal_lastname', 'oncreate', 'auth_kmsh');
        set_config('field_lock_lastname', 'unlocked', 'auth_kmsh');
        // Setu up field mappings.

        set_config('field_map_email', 'email', 'auth_kmsh');
        set_config('field_updatelocal_email', 'oncreate', 'auth_kmsh');
        set_config('field_updateremote_email', '0', 'auth_kmsh');
        set_config('field_lock_email', 'unlocked', 'auth_kmsh');
        ...
        set_config('field_map_profile_field_pet', 'animal', 'auth_kmsh');
        set_config('field_updatelocal_profile_field_pet', 'oncreate', 'auth_kmsh');
        set_config('field_updateremote_profile_field_pet', '0', 'auth_kmsh');
        set_config('field_lock_profile_field_pet', 'unlocked', 'auth_kmsh');

        // Init the rest of settings.
        set_config('passtype', 'plaintext', 'auth_kmsh');
        set_config('changepasswordurl', '', 'auth_kmsh');
        set_config('debugauthdb', 0, 'auth_kmsh');
        set_config('removeuser', AUTH_REMOVEUSER_KEEP, 'auth_kmsh');
    }
    ...
    public function test_plugin() {
        ...
        $user1->id = $DB->insert_record('auth_kmsh_users', $user1);
        ...
        $user2->id = $DB->insert_record('auth_kmsh_users', $user2);

        $user3 = (object)array('name'=>'admin', 'pass'=>'heslo', 'email'=>'admin@example.com'); // Should be skipped.
        $user3->id = $DB->insert_record('auth_kmsh_users', $user3);
        ...
        $u1 = $DB->get_record('user', array('username'=>$user1->name, 'auth'=>'kmsh'));
        $this->assertSame($user1->email, $u1->email);
        $this->assertEmpty(profile_user_record($u1->id)->pet);
        $u2 = $DB->get_record('user', array('username'=>$user2->name, 'auth'=>'kmsh'));
        ...
        $user2b->animal = 'dog';
        $DB->update_record('auth_kmsh_users', $user2b);
        ...
        set_config('field_updatelocal_email', 'onlogin', 'auth_kmsh');
        $auth->config->field_updatelocal_email = 'onlogin';
        set_config('field_updatelocal_profile_field_pet', 'onlogin', 'auth_kmsh');
        ...
        $DB->delete_records('auth_kmsh_users', array('id'=>$user2->id));
        $this->assertCount(2, $DB->get_records('auth_kmsh_users'));
        unset($user2);
        unset($user2b);
        ...
        set_config('removeuser', AUTH_REMOVEUSER_SUSPEND, 'auth_kmsh');
        ...
        set_config('removeuser', AUTH_REMOVEUSER_FULLDELETE, 'auth_kmsh');
        ...
        $this->assertFalse($DB->record_exists('user', array('username'=>'u3', 'auth'=>'kmsh')));
        $this->assertTrue($auth->user_login('u3', 'heslo'));
        $this->assertFalse($DB->record_exists('user', array('username'=>'u3', 'auth'=>'kmsh')));

        set_config('passtype', 'md5', 'auth_kmsh');
        $auth->config->passtype = 'md5';
        $user3->pass = md5('heslo');
        $DB->update_record('auth_kmsh_users', $user3);
        $this->assertTrue($auth->user_login('u3', 'heslo'));
    ...
    public function test_clean_data() {
        ...
        $extdbuser1->id = $DB->insert_record('auth_kmsh_users', $extdbuser1);
        ...
        $extdbuser2->id = $DB->insert_record('auth_kmsh_users', $extdbuser2);

        $extdbuser3 = (object)array('name'=>'u3', 'pass'=>'heslo', 'email'=>'u3@example.com',
                'lastname' => 'user<script>alert(1);</script>xss');
        $extdbuser3->id = $DB->insert_record('auth_kmsh_users', $extdbuser3);
        $trace = new null_progress_trace();
        ...
        $user1 = $DB->get_record('user', array('email'=> $extdbuser1->email, 'auth'=>'kmsh'));
        ...
        $user2 = $DB->get_record('user', array('email'=> $extdbuser2->email, 'auth'=>'kmsh'));
        $user3 = $DB->get_record('user', array('email'=> $extdbuser3->email, 'auth'=>'kmsh'));
    ...
    public function test_deleting_with_many_users() {
        ...
        for ($i = 0; $i < $this->largedeletionsetsize; $i++) {
            ...
            $user->id  = $DB->insert_record('auth_kmsh_users', $user);
            $users[] = $user;
        }
        ...
        $moodleuser = $DB->get_record('user', array('email' => $user->email, 'auth' => 'kmsh'));
        $this->assertNotNull($moodleuser);
        $this->assertEquals($user->username, $moodleuser->username);

        // Delete a user.
        $DB->delete_records('auth_kmsh_users', array('id' => $user->id));
        ...
        $numberdeleted = $DB->count_records('user', array('deleted' => 1, 'auth' => 'kmsh'));
