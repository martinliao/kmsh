...
class admin_uploaduser_form1 extends moodleform {
    function definition () {
        ...
        */
        ...
        if($dir = opendir($CFG->dirroot.'/admin/tool/uploaduser/templatefiles')){
            $templatefile = '';
            while (false !== ($template = readdir($dir))) {
                if($template == "." || $template =="..") {
                    continue;
                }
                 $url = new moodle_url('/admin/tool/uploaduser/templatefiles/'.$template);
                 $templatefile .= html_writer::link($url, $template)."&nbsp;&nbsp;";
            }
            $mform->addElement('static', 'templatefile', get_string('templatefile', 'tool_uploaduser'), $templatefile);
        }
