...
/**
 * Program award criteria -- award on course completion
 *
 */
class program_award_criteria_course extends program_award_criteria {

    /* @var int Criteria [PROGRAM_CRITERIA_TYPE_COURSE] */
    public $criteriatype = PROGRAM_CRITERIA_TYPE_COURSE;
    ...
    public function __construct($record) {
        ...
        $this->course = $DB->get_record_sql('SELECT c.id, c.enablecompletion, c.cacherev, c.startdate
                        FROM {program} b INNER JOIN {course} c ON b.courseid = c.id
                        WHERE b.id = :programid ', array('programid' => $this->programid), MUST_EXIST);
    ...
    public function config_form_criteria($data) {
        ...
        $editurl = new moodle_url('/admin/clickap/program/criteria_settings.php', array('programid' => $this->programid, 'edit' => true, 'type' => $this->criteriatype, 'crit' => $this->id));
        $deleteurl = new moodle_url('/admin/clickap/program/criteria_action.php', array('programid' => $this->programid, 'delete' => true, 'type' => $this->criteriatype));
        ...
        if (!empty($this->params)) {
            echo $OUTPUT->box(get_string('criteria_descr_' . $this->criteriatype, 'clickap_program') . $this->get_details(), array('clearfix'));
        }
        echo $OUTPUT->box_end();
    }
    ...
    public function get_details($short = '') {
        ...
        if (!$course) {
            ...
            if (isset($param['bydate'])) {
                $str .= get_string('criteria_descr_bydate', 'clickap_program', userdate($param['bydate'], get_string('strftimedate', 'core_langconfig')));
            }
            if (isset($param['grade'])) {
                $str .= get_string('criteria_descr_grade', 'clickap_program', $param['grade']);
            }
        }
        return $str;
    }
    ...
    public function get_options(&$mform) {
        ...
        if (!($course->enablecompletion == COMPLETION_ENABLED)) {
            $none = true;
            $message = get_string('completionnotenabled', 'clickap_program');
        } else {
            $mform->addElement('header', 'criteria_course', $this->get_title());
            $mform->addHelpButton('criteria_course', 'criteria_' . $this->criteriatype, 'clickap_program');
            $parameter = array();
            $parameter[] =& $mform->createElement('static', 'mgrade_', null, get_string('mingrade', 'clickap_program'));
            $parameter[] =& $mform->createElement('text', 'grade_' . $param['course'], '', array('size' => '5'));
            $parameter[] =& $mform->createElement('static', 'complby_' . $param['course'], null, get_string('bydate', 'clickap_program'));
            ...
            $mform->addElement('hidden', 'agg', PROGRAM_CRITERIA_AGGREGATION_ALL);
