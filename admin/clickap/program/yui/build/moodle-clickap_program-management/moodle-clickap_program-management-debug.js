YUI.add('moodle-clickap_program-management', function (Y, NAME) {
 ...
 *
 * @module moodle-clickap_program-management
 */

function Console() {
    Console.superclass.constructor.apply(this, arguments);
}
Console.NAME = 'moodle-clickap_program-management';
...
Console.ATTRS = {
     ...
     * @attribute ajaxurl
     * @type String
     * @default /admin/clickap/program/category/ajax/management.php
     */
    ajaxurl : {
        getter : function(value) {
            if (value === null) {
                value = M.cfg.wwwroot + '/admin/clickap/program/category/ajax/management.php';
            }
            return value;
        },
        value : null
...
Console.prototype = {
    ...
    initializer : function() {
        Y.log('Initialising course category management console', 'info', 'moodle-clickap_program-management');
    ...
    initialiseCategories : function(listing) {
        ...
        if (!this.categoriesinit) {
            ...
            Y.log(count+' categories being managed', 'info', 'moodle-clickap_program-management');
        } else {
            Y.log(count+' new categories being managed', 'info', 'moodle-clickap_program-management');
        }
    },
    ...
    initialiseCourses : function() {
        ...
        Y.log(count+' courses being managed', 'info', 'moodle-clickap_program-management');
    },
    ...
    handleCourseDelegation : function(e) {
        ...
        if (course) {
            ...
            Y.log('Course with ID '+courseid+' could not be found for delegation', 'error', 'moodle-clickap_program-management');
        }
    },
    ...
    handleCategoryDelegation : function(e) {
        ...
        if (category) {
            ...
            Y.log('Could not find category to delegate to.', 'error', 'moodle-clickap_program-management');
        }
    },
    ...
    performAjaxAction : function(action, args, callback, context) {
        ...
        if (callback === null) {
            callback = function() {
                Y.log("'Action '"+action+"' completed", 'debug', 'moodle-clickap_program-management');
            };
        }
...
M.clickap_program = M.clickap_program || {};
M.clickap_program.management = M.clickap_program.management || {};
M.clickap_program.management.console = null;

/**
 * Initalises the course management console.
 *
 * @method M.clickap_program.management.init
 * @static
 * @param {Object} config
 */
M.clickap_program.management.init = function(config) {
    M.clickap_program.management.console = new Console(config);
};
/* global Console */
...
DragDrop.NAME = 'moodle-clickap_program-management-dd';
...
DragDrop.prototype = {
    ...
    dropHit : function(e) {
        ...
        if (!drag.test('.listitem-course')) {
            Y.log('It was not a course being dragged.', 'warn', 'moodle-clickap_program-management');
            return false;
        }
        ...
        if (iscategory) {
            ...
            Y.log('Course dropped over unhandled target.', 'info', 'moodle-clickap_program-management');
        }
    }
};
...
Item.NAME = 'moodle-clickap_program-management-item';
...
Item.prototype = {
    ...
    moveup : function(transactionid, response, args) {
        ...
        if (outcome === false) {
            Y.log('AJAX request to move '+this.get('itemname')+' up failed by outcome.', 'warn', 'moodle-clickap_program-management');
            return false;
        }
        node = this.get('node');
        ...
        if (previous) {
            ...
            this.updated(true);
            Y.log('Success: '+this.get('itemname')+' moved up by AJAX.', 'info', 'moodle-clickap_program-management');
        } else {
            ...
            Y.log(this.get('itemname') + ' cannot be moved up as its the top item on this page.',
                    'info', 'moodle-clickap_program-management');
    ...
    movedown : function(transactionid, response, args) {
        var node,
            ...
            outcome = this.checkAjaxResponse(transactionid, response, args);
        if (outcome === false) {
            Y.log('AJAX request to move '+this.get('itemname')+' down failed by outcome.', 'warn', 'moodle-clickap_program-management');
            return false;
        }
        node = this.get('node');
        ...
        if (next) {
            ...
            this.updated(true);
            Y.log('Success: '+this.get('itemname')+' moved down by AJAX.', 'info', 'moodle-clickap_program-management');
        } else {
    ...
    show : function(transactionid, response, args) {
        var outcome = this.checkAjaxResponse(transactionid, response, args),
            hidebtn;
        if (outcome === false) {
            Y.log('AJAX request to show '+this.get('itemname')+' by outcome.', 'warn', 'moodle-clickap_program-management');
            return false;
        }
        ...
        this.updated();
        Y.log('Success: '+this.get('itemname')+' made visible by AJAX.', 'info', 'moodle-clickap_program-management');
    },
    ...
    markVisible : function() {
        ...
        Y.log('Marked '+this.get('itemname')+' as visible', 'info', 'moodle-clickap_program-management');
        return true;
    },
    ...
    hide : function(transactionid, response, args) {
        var outcome = this.checkAjaxResponse(transactionid, response, args),
            showbtn;
        if (outcome === false) {
            Y.log('AJAX request to hide '+this.get('itemname')+' by outcome.', 'warn', 'moodle-clickap_program-management');
            return false;
        }
        this.markHidden();
        ...
        this.updated();
        Y.log('Success: '+this.get('itemname')+' made hidden by AJAX.', 'info', 'moodle-clickap_program-management');
    },
    ...
    markHidden : function() {
        ...
        Y.log('Marked '+this.get('itemname')+' as hidden', 'info', 'moodle-clickap_program-management');
        return true;
    },
...
Category.NAME = 'moodle-clickap_program-management-category';
...
Category.ATTRS = {
    ...
    programid : {
        getter : function (value, name) {
            if (value === null) {
                value = this.get('node').getData('programid');
                this.set(name, value);
            }
            return value;
        },
...
Category.prototype = {
    ...
    handle : function(action, e) {
        var catarg = {programid : this.get('programid'), categoryid : this.get('categoryid')},
        ...
        switch (action) {
            ...
            case 'select':
                ...
                break;
            default:
                Y.log('Invalid AJAX action requested of managed category.', 'warn', 'moodle-clickap_program-management');
                return false;
        }
    },
    ...
    loadSubcategories : function(transactionid, response, args) {
        var outcome = this.checkAjaxResponse(transactionid, response, args),
            ...
            actionnode;
        if (outcome === false) {
            Y.log('AJAX failed to load sub categories for '+this.get('itemname'), 'warn', 'moodle-clickap_program-management');
            return false;
        }
        Y.log('AJAX loaded subcategories for '+this.get('itemname'), 'info', 'moodle-clickap_program-management');
    ...
    completeMoveCourse : function(transactionid, response, args) {
        var outcome = this.checkAjaxResponse(transactionid, response, args),
            ...
            totals;
        if (outcome === false) {
            Y.log('AJAX failed to move courses into this category: '+this.get('itemname'), 'warn', 'moodle-clickap_program-management');
            return false;
        }
        course = managementconsole.getCourseById(args.courseid);
        if (!course) {
            Y.log('Course was moved but the course listing could not be found to reflect this', 'warn', 'moodle-clickap_program-management');
            return false;
        }
        Y.log('Moved the course (' + course.getName() + ') into this category (' + this.getName() + ')',
            'debug', 'moodle-clickap_program-management');
        this.highlight();
        if (course) {
    ...
    show : function(transactionid, response, args) {
        ...
        this.updated();
        Y.log('Success: category made visible by AJAX.', 'info', 'moodle-clickap_program-management');
    },
    ...
    updateCourseVisiblity : function(courses) {
        var managementconsole = this.get('console'),
            key,
            course;
        Y.log('Changing categories course visibility', 'info', 'moodle-clickap_program-management');
        try {
            for (key in courses) {
            ...
            Y.log('Error trying to update course visibility: ' + err.message, 'warn', 'moodle-clickap_program-management');
        }
        return this;
    },
    ...
    updateChildVisibility : function(categories) {
        var managementconsole = this.get('console'),
            key,
            category;
        Y.log('Changing categories subcategory visibility', 'info', 'moodle-clickap_program-management');
        try {
            ...
            Y.log('Error trying to update category visibility: ' + err.message, 'warn', 'moodle-clickap_program-management');
        }
        return this;
    }
};
...
Course.NAME = 'moodle-clickap_program-management-course';
...
Course.ATTRS = {
    ...
    categoryid : {
        getter : function (value, name) {
            if (value === null) {
                value = this.get('node').getData('categoryid');
                this.set(name, value);
            }
            return value;
        },
...
Course.prototype = {
    ...
    handle : function(action, e) {
        var managementconsole = this.get('console'),
            args = {programid : this.get('programid'),categoryid : this.get('categoryid'),courseid : this.get('courseid')};
        switch (action) {
            case 'moveup':
                e.halt();
            ...
            case 'select':
                ...
                break;
            default:
                Y.log('Invalid AJAX action requested of managed course.', 'warn', 'moodle-clickap_program-management');
                return false;
        }
    },
    ...
    moveAfterResponse : function(transactionid, response, args) {
        ...
        if (outcome === false) {
            ...
            Y.log('AJAX failed to move this course after the requested course', 'warn', 'moodle-clickap_program-management');
            if (previous) {
        ...
        Y.log('AJAX successfully moved course ('+this.getName()+')', 'info', 'moodle-clickap_program-management');
        this.highlight();
    }
};
