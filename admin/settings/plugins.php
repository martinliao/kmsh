...
// Add all clickap
if ($hassiteconfig || has_capability('clickap/base:config', $systemcontext)) {
    $ADMIN->add('modules', new admin_category('clickap', new lang_string('clickap', 'clickap_base')));
    $ADMIN->add('clickap', new admin_externalpage('manageclickap', new lang_string('clickapmanage', 'clickap_base'), $CFG->wwwroot . '/' . $CFG->admin . '/clickap.php'));
}

// Now add various clickap.
$clickapplugin = core_plugin_manager::instance()->get_plugins_of_type('clickap');
if(array_key_exists("base", $clickapplugin)){
    $plugin = $clickapplugin['base'];
    $plugin->load_settings($ADMIN, 'clickapsettings', has_capability('clickap/base:config', $systemcontext));
}
foreach (core_plugin_manager::instance()->get_plugins_of_type('clickap') as $plugin) {
    if($plugin->name == 'base'){
        continue;
    }
    /** @var \core\plugininfo\clickap $plugin */
    $plugin->load_settings($ADMIN, 'clickapsettings', has_capability('clickap/base:config', $systemcontext));
}
